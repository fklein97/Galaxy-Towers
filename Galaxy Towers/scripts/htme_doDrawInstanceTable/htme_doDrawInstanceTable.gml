/// @description htme_debugoverlayDrawInstanceTable(title,type,filter,drawInRoom);
/// @param title
/// @param type
/// @param filter
/// @param drawInRoom
...
if (self.dbgcntx2 != "") {
   //DETAILS
    ...
    var base = self.dbg_top+20+string_height(string_hash_to_newline(headstr))+10;
    ...
    if (isExisting && isVisible) {
        ...
        draw_text((instance).x-(instance).sprite_xoffset+(instance).sprite_width,(instance).y-(instance).sprite_yoffset+(instance).sprite_height,string_hash_to_newline(insttext));
        if (isLocal) draw_set_colour(self.dbgcolor);
    }
    
    draw_text(self.dbg_left+20,offs,string_hash_to_newline(str));
} else {
    //INSTANCE TABLE
    ...
    for(var i=0; i<ds_map_size(mapToUse); i+=1) {
        ...
        if (isExisting && isVisible && drawInRoom) {
            if (isLocal) draw_set_colour(self.dbgcolor_a);
            draw_text((inst).x-(inst).sprite_xoffset+(inst).sprite_width,(inst).y-(inst).sprite_yoffset+(inst).sprite_height,string_hash_to_newline(insttext));
            if (isLocal) draw_set_colour(self.dbgcolor);
        }
        ...
        //Add to summary
        draw_text(self.dbg_left+20,offs,string_hash_to_newline(instsummary));
        offs = offs + string_height(string_hash_to_newline(instsummary));
        
        key = ds_map_find_next(mapToUse, key);
    }
}
//HEADER
draw_rectangle_colour(self.dbg_left,self.dbg_top,self.dbg_left+20+string_width(string_hash_to_newline(headstr))+20,self.dbg_top+20+string_height(string_hash_to_newline(headstr))+5,c_black,c_black,c_black,c_black,false);
draw_text(self.dbg_left+20,self.dbg_top+20,string_hash_to_newline(headstr));
